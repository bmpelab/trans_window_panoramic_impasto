...
%%
function [E,DE] = dM_ver5(HYPERMAP,CCMASK,H,W)
...
ccmask = CCMASK(dH+1:dH+H,dW+1:dW+W);
IMAP(ccmask) = 0;
DMAP = HYPERMAP.derivative_map(dH+1:dH+H,dW+1:dW+W,:);
SMAP = HYPERMAP.derivative_direction_map(dH+1:dH+H,dW+1:dW+W,:);
valid_flag = logical((IMAP(:)~=0).*(~ccmask(:)));
N = sum(valid_flag);
