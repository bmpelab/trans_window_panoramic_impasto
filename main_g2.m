...
disp("Have you checked the image_offset, sampling rate, constraint folder, mask folder, output folder?")
pause();
...
data_folder = '../../Data/202312141727_g2_scan_and_video';
left_image_folder = [data_folder '/rectified_left'];
tool_mask_folder = [data_folder '/mask_d5'];
...
% output mesh folder
% mesh_folder = [data_folder '/mesh'];
% texture_mesh_folder = [data_folder '/mesh_texture_730-900'];
hypermap_folder = ['../../Data/202402061620_hypermap_g1g2/g2/hypermap'];
texture_mesh_folder = ['../../Data/202402061620_hypermap_g1g2/g2/mesh_texture_1'];
if ~exist(hypermap_folder,'dir')
    mkdir(hypermap_folder);
end
if ~exist(texture_mesh_folder,'dir')
    mkdir(texture_mesh_folder);
end

left_image = imread([left_image_folder '/' left_image_names{ia}]);
left_image = left_image(:,1+image_offset:end,:);
left_image = left_image(1:sr:end,1:sr:end,:);
point_3d_map = load([point_3d_map_folder '/' point_3d_map_names{ia}]).point_3d_map;
...
%
for i = ia : ib% length(scene_flow_map_names)
    disp(i);
...
%     tool_mask_b = logical(tool_mask_b);
    % point 3d map
...
%     [hypermap,pe,forward_scene_flow_hyper] = saveForwardSceneFlow_ver2(hypermap,pe,cc_map,cc_mask,cc_group_id,cc_centers,iso_flags,alpha,pose);
    ...
    % save hyper map for the i-th frame !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    % save mesh_a
    if save_flag
        if i == ia
    ...
    if texture_flag && save_flag
        ...
        save([hypermap_folder '/' erase(point_3d_map_names{i},'mat') 'mat'], "hypermap_output");
    end
...
%     [point_3d_map_para,derivative_map_para,color_map_para,backward_scene_flow_map_para,mask_para,bindex_map] = ...
%         parameterization_ver4(height,width,heightc,widthc,pe,faces_hyper,derivatives_hyper,colors_hyper,...
%         -forward_scene_flow_hyper,bindices,PL,sr,image_offset);
    [point_3d_map_para,derivative_map_para,color_map_para,backward_scene_flow_map_para,backward_optical_flow_map_para,mask_para,bindex_map] = ...
        parameterization_ver5(height,width,heightc,widthc,pe,faces_hyper,derivatives_hyper,colors_hyper,...
        -forward_scene_flow_hyper,-forward_optical_flow_hyper,bindices,PL,sr,image_offset);
    % merge
%     [smooth_mask] = smoothMaskGenerator(mask_para,~logical(vertex_index_map_b));
    ...
    if save_flag
        ...
        [edge_mask_output,vertex_index_map_output,~,~,~,~,~,~,~]...
            = surfaceReconstructionFromPoint3DMap_ver6(point_3d_map_hyper,point_mask_hyper,[],1,false,true,1*edge_filter_threshold,height*width/100);
    ...
    if i == ib
        ...
        if texture_flag && save_flag
            ...
            save([hypermap_folder '/' erase(point_3d_map_names{i+1},'mat') 'mat'], "hypermap_output");
        end
    end
end
%% generate image plane point cloud
module_path = '../../Module/matlab_module';
addpath(genpath([module_path '/natural_sort_0.1.0']));

data_folder = '../../Data/202402081824_clip_hamlyn_porcine_slam_low_defor';
left_image_folder = [data_folder '/rectified_left'];
sr = 2;
...
% output point cloud folder
image_pointcloud_folder = [data_folder '/image_ply'];
if ~exist(image_pointcloud_folder,'dir')
    mkdir(image_pointcloud_folder);
end
...
point_3d_map = zeros(height,width,3);
parfor r = 1 : height
    for c = 1 : width
        z = 50;
        u = (c-1)*sr;
        ...
        x = (u-cx)*z/f;
        y = (v-cy)*z/f;
        point_3d_map(r,c,:) = [x y z];
    end
end

for i = 1 : length(left_image_names)
    left_image = imread([left_image_folder '/' left_image_names{i}]);
    left_image = left_image(1:sr:end,1:sr:end,:);
    pcwrite(pointCloud(point_3d_map,Color=left_image),[image_pointcloud_folder '/' erase(left_image_names{i},'png') 'ply']);
end
%% generate canonical image
module_path = '../../Module/matlab_module';
addpath(genpath([module_path '/natural_sort_0.1.0']));

data_folder = '../../Data/202402061620_hypermap_g1g2/g2';
mesh_folder = [data_folder '/_mesh_texture_1'];

% load([data_folder '/rectifiedCamera.mat']);
load("F:\chen\Documents\DataManagement\Data\202312141727_g2_scan_and_video\rectifiedCamera.mat");

% mesh
mesh_dir = dir([mesh_folder '/*.ply']);
mesh_names = {mesh_dir.name};
[mesh_names,~] = sortNat(mesh_names);

% output image folder
canonical_image_folder = [data_folder '/canonical_image'];
if ~exist(canonical_image_folder,'dir')
    mkdir(canonical_image_folder);
end

sr = 3;
offset = 64;
height = length(1:sr:720);
width = length(1:sr:(1280-offset));
heightc = round(height*1.2);
widthc = round(width*1.2);
dh = round((heightc-height)/2);
dw = round((widthc-width)/2);

parfor i = 1 : length(mesh_names)
    mesh = pcread([mesh_folder '/' mesh_names{i}]);
    canonical_image = uint8(zeros(heightc,widthc,3));
    for j = 1 : mesh.Count
        vertex = mesh.Location(j,:);
        pixel = PL*[vertex';1];
        pixel = pixel/pixel(3);
        pixel(1) = pixel(1)-offset;
        pixel = pixel/sr;
        cr = round(pixel(1:2)+1);
        canonical_image(cr(2)+dh,cr(1)+dw,:) = mesh.Color(j,:);
    end
    imwrite(canonical_image,[canonical_image_folder '/' erase(mesh_names{i},'ply') 'png']);
end

%% generate strain point cloud and image
module_path = '../../Module/matlab_module';
addpath(genpath([module_path '/natural_sort_0.1.0']));

data_folder = '../../Data/202402061620_hypermap_g1g2/g2';
hypermap_folder = [data_folder '/_hypermap'];

% load([data_folder '/rectifiedCamera.mat']);
load("F:\chen\Documents\DataManagement\Data\202312141727_g2_scan_and_video\rectifiedCamera.mat");

% hypermap
hypermap_dir = dir([hypermap_folder '/*.mat']);
hypermap_names = {hypermap_dir.name};
[hypermap_names,~] = sortNat(hypermap_names);

ia = 1;

% output strain ply folder
strain_ply_folder = [data_folder '/strain_ply'];
if ~exist(strain_ply_folder,'dir')
    mkdir(strain_ply_folder);
end
% output strain map folder
strain_map_folder = [data_folder '/strain_map'];
if ~exist(strain_map_folder,'dir')
    mkdir(strain_map_folder);
end

roi_mask0 = imread([data_folder '/roi_mask_0.png']); % roi defined in the canonical space
roi_mask0 = logical((roi_mask0(:,:,1)==0).*(roi_mask0(:,:,2)==255).*(roi_mask0(:,:,3)==0)); % == true means roi
roi_mask = roi_mask0;
[rs0,cs0] = find(roi_mask0);
rs = rs0;
cs = cs0;
tracked_flag = true(length(rs0),1); % == true means under tracking
hypermap0 = load([hypermap_folder '/' hypermap_names{ia}]).hypermap_output;
accumulated_forward_scene_flow_map = zeros(size(hypermap0.forward_scene_flow_map));
tracked_point_3d_map = zeros(size(hypermap0.point_3d_map));

height = size(hypermap0.point_3d_map,1);
width = size(hypermap0.point_3d_map,2);
...
valid_flag = roi_mask0(:); % == true is valid
tracked_points = reshape(hypermap0.point_3d_map,[],3);
tracked_points = tracked_points(valid_flag,:);
strain_map = zeros(size(roi_mask0));
strain = strain_map(:);
strain(strain>max_strain) = max_strain;
strain(strain<min_strain) = min_strain;
color_indices = round((strain-min_strain)/(max_strain-min_strain)*255+1);
colors_tri = cmap(color_indices,:);
colors_tri(~valid_flag,:) = 0;
strain_ply = pointCloud(tracked_points,'Color',uint8(colors_tri(valid_flag,:)*255));
strain_color_map = reshape(colors_tri,height,width,[]);
imwrite(uint8(strain_color_map*255),[strain_map_folder '/' erase(hypermap_names{ia},'mat') 'png']);
pcwrite(strain_ply,[strain_ply_folder '/' erase(hypermap_names{ia},'mat') 'ply']);

for i = ia : length(hypermap_names)-1
    hypermap = load([hypermap_folder '/' hypermap_names{i}]).hypermap_output;
    % update accumulated_forward_scene_flow_map and tracked_point_3d_map
    for j = 1 : length(rs0)
        % lost of tracking
        if ~tracked_flag(j)
            continue;
        ...
        cb = ceil(cs(j));
        %
        if hypermap.vertex_index_map(ra,ca)==0 ...
            || hypermap.vertex_index_map(rb,ca)==0 ...
            || hypermap.vertex_index_map(rb,cb)==0 ...
            || hypermap.vertex_index_map(ra,cb)==0
            tracked_flag(j) = false;
            roi_mask(rs0(j),cs0(j)) = false;
            accumulated_forward_scene_flow_map(rs0(j),cs0(j),:) = 0;
            tracked_point_3d_map(rs0(j),cs0(j),:) = 0;
            continue;
        end
        %
        if ra==rb && ca==cb
            accumulated_forward_scene_flow_map(rs0(j),cs0(j),:) = accumulated_forward_scene_flow_map(rs0(j),cs0(j),:) ...
                + hypermap.forward_scene_flow_map(rs(j),cs(j),:);
            tracked_point_3d_map(rs0(j),cs0(j),:) = hypermap.point_3d_map(rs(j),cs(j),:);
        elseif ra==rb
            wx = [cs(j)-ca cb-cs(j)]; % already normalized
            accumulated_forward_scene_flow_map(rs0(j),cs0(j),:) = accumulated_forward_scene_flow_map(rs0(j),cs0(j),:) ...
                + wx(1)*hypermap.forward_scene_flow_map(rs(j),cb,:) + wx(2)*hypermap.forward_scene_flow_map(rs(j),ca,:);
            tracked_point_3d_map(rs0(j),cs0(j),:) = wx(1)*hypermap.point_3d_map(rs(j),cb,:) + wx(2)*hypermap.point_3d_map(rs(j),ca,:);
        elseif ca==cb
            wy = [rs(j)-ra rb-rs(j)];
            accumulated_forward_scene_flow_map(rs0(j),cs0(j),:) = accumulated_forward_scene_flow_map(rs0(j),cs0(j),:) ...
                + wy(1)*hypermap.forward_scene_flow_map(rb,cs(j),:) + wy(2)*hypermap.forward_scene_flow_map(ra,cs(j),:);
            tracked_point_3d_map(rs0(j),cs0(j),:) = wy(1)*hypermap.point_3d_map(rb,cs(j),:) + wy(2)*hypermap.point_3d_map(ra,cs(j),:);
        else
            wx = [cs(j)-ca cb-cs(j)]; % already normalized
            wy = [rs(j)-ra rb-rs(j)];
            accumulated_forward_scene_flow_map(rs0(j),cs0(j),:) = accumulated_forward_scene_flow_map(rs0(j),cs0(j),:) ...
                + wy(1)*(wx(1)*hypermap.forward_scene_flow_map(rb,cb,:) + wx(2)*hypermap.forward_scene_flow_map(rb,ca,:)) ...
                + wy(2)*(wx(1)*hypermap.forward_scene_flow_map(ra,cb,:) + wx(2)*hypermap.forward_scene_flow_map(ra,ca,:));
            tracked_point_3d_map(rs0(j),cs0(j),:) = ...
                + wy(1)*(wx(1)*hypermap.point_3d_map(rb,cb,:) + wx(2)*hypermap.point_3d_map(rb,ca,:)) ...
                + wy(2)*(wx(1)*hypermap.point_3d_map(ra,cb,:) + wx(2)*hypermap.point_3d_map(ra,ca,:));
        end
    end
    % estimate the strain
    [strain_mask,strain_map] = maxPrincipalStrainMaskGenerator(accumulated_forward_scene_flow_map,hypermap0.point_3d_map,~roi_mask,inf);
    % reconstruct the strain ply
    valid_flag = ~strain_mask(:); % == true is valid
    tracked_points = reshape(tracked_point_3d_map,[],3);
    tracked_points = tracked_points(valid_flag,:);
    strain = strain_map(:);
    strain(strain>max_strain) = max_strain;
    strain(strain<min_strain) = min_strain;
    ...
    strain_ply = pointCloud(tracked_points,'Color',uint8(colors_tri(valid_flag,:)*255));
    strain_color_map = reshape(colors_tri,height,width,[]);
    imwrite(uint8(strain_color_map*255),[strain_map_folder '/' erase(hypermap_names{i+1},'mat') 'png']);
    pcwrite(strain_ply,[strain_ply_folder '/' erase(hypermap_names{i+1},'mat') 'ply']);
    % track the next
    for j = 1 : length(rs0)
        % lost of tracking
        if ~tracked_flag(j)
        ...
        %
        if ra==rb && ca==cb
            rs(j) = rs(j) + hypermap.forward_optical_flow_map(ra,ca,2);
            cs(j) = cs(j) + hypermap.forward_optical_flow_map(ra,ca,1);
        elseif ra==rb
            wx = [cs(j)-ca cb-cs(j)]; % already normalized
            rs(j) = rs(j) ...
                + wx(1)*hypermap.forward_optical_flow_map(ra,cb,2) + wx(2)*hypermap.forward_optical_flow_map(ra,ca,2);
            cs(j) = cs(j) ...
                + wx(1)*hypermap.forward_optical_flow_map(ra,cb,1) + wx(2)*hypermap.forward_optical_flow_map(ra,ca,1);
        elseif ca==cb
            wy = [rs(j)-ra rb-rs(j)];
            rs(j) = rs(j) ...
                + wy(1)*hypermap.forward_optical_flow_map(rb,ca,2) + wy(2)*hypermap.forward_optical_flow_map(ra,ca,2);
            cs(j) = cs(j) ...
                + wy(1)*hypermap.forward_optical_flow_map(rb,ca,1) + wy(2)*hypermap.forward_optical_flow_map(ra,ca,1);
        else
            wx = [cs(j)-ca cb-cs(j)]; % already normalized
            wy = [rs(j)-ra rb-rs(j)];
            rs(j) = rs(j) ...
                + wy(1)*(wx(1)*hypermap.forward_optical_flow_map(rb,cb,2) + wx(2)*hypermap.forward_optical_flow_map(rb,ca,2)) ...
                + wy(2)*(wx(1)*hypermap.forward_optical_flow_map(ra,cb,2) + wx(2)*hypermap.forward_optical_flow_map(ra,ca,2));
            cs(j) = cs(j) ...
                + wy(1)*(wx(1)*hypermap.forward_optical_flow_map(rb,cb,1) + wx(2)*hypermap.forward_optical_flow_map(rb,ca,1)) ...
                + wy(2)*(wx(1)*hypermap.forward_optical_flow_map(ra,cb,1) + wx(2)*hypermap.forward_optical_flow_map(ra,ca,1));
        end
    end
end
